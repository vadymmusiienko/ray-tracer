            ...
            Parser.Default
                ...
                .WithParsed(options =>
                {
                    try
                    {
                        ...
                        // Construct the scene from either the modern JSON format
                        // or the legacy text DSL, based on the file extension.
                        Scene scene;
                        int width = options.OutputImageWidth;
                        int height = options.OutputImageHeight;

                        if (options.InputFilePath.EndsWith(".json", StringComparison.OrdinalIgnoreCase))
                        {
                            // JSON scenes are self-contained: render settings and
                            // dimensions come from the file, not the CLI flags.
                            var assets = AssetResolver.FromDirectories("tests/textures", "tests/models");
                            string json = File.ReadAllText(options.InputFilePath);
                            SceneBuildResult result = JsonSceneLoader.Load(json, assets);
                            scene = result.Scene;
                            width = result.Width;
                        ...
                        {
                            // Legacy text DSL: render settings come from CLI flags.
                            var sceneReader = new SceneReader(options.InputFilePath);
                            scene = new Scene(new SceneOptions(
