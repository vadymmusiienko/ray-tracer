...
namespace RayTracer
{
    ...
    public class Camera : SceneEntity
    {
        ...
        public static Vector3 sampleAperture(double radius)
        {
            ...
            // Random.Shared is thread-safe, so DOF sampling is safe under the
            // parallelized render loop (a plain static Random would corrupt).
            double r = radius * Math.Sqrt(Random.Shared.NextDouble()); // Square root for uniform area distribution
            double theta = Random.Shared.NextDouble() * 2.0 * Math.PI;
